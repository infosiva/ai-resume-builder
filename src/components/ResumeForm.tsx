...
export default function ResumeForm({ onGenerate, setLoading, onAnalysis }: Props) {
  ...
  return (
    <div className="rounded-2xl border border-white/10 bg-white/[0.03] backdrop-blur-xl p-8">
      ...
      <p className="text-sm text-white/40 mb-6">Paste a job spec · AI analyses the match · Get a tailored resume</p>
      ...
      <form onSubmit={handleSubmit} className="space-y-5">
        ...
        <div className="flex gap-3">
          <button
            ...
            disabled={!jobDesc || !experience || analyzing}
            className="flex-1 py-3 rounded-xl border border-violet-500/40 bg-violet-500/10 hover:bg-violet-500/20 text-violet-300 font-medium text-sm transition-all disabled:opacity-40 flex items-center justify-center gap-2"
          >
            {analyzing ? (
              <><div className="w-4 h-4 border-2 border-violet-400/30 border-t-violet-400 rounded-full animate-spin" /> Analysing match...</>
            ) : (
              <><span>⚡</span> Check job match</>
            )}
          </button>

          <button
            type="submit"
            disabled={!jobDesc || !experience}
            className="flex-1 py-3 rounded-xl bg-gradient-to-r from-violet-600 to-cyan-600 hover:from-violet-500 hover:to-cyan-500 font-semibold text-sm transition-all shadow-lg shadow-violet-500/20 flex items-center justify-center gap-2 disabled:opacity-40"
          >
